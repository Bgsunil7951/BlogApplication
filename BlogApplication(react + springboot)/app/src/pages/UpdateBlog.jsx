...
const UpdateBlog = () => {
  ...
  const updateBlog = async (val) => {
    const response = await axiosInstance.put(`/blog/secure/update/${BLOG_DATA?.id}`, val);
    return response.data;
  };

  const deleteBlog = async (val) => {
    const response = await axiosInstance.delete(`/blog/secure/delete/${BLOG_DATA?.id}`, val);
    return response.data;
  };
  ...
  return (
    <>
      ...
      <article className="w-full flex justify-center h-[60vh] items-center">
        <article className="p-3 w-full max-w-[350px] shadow-xl rounded-2xl">
          <div className="flex justify-end items-center w-full my-5">
            {
                deleteBlogMutation?.isLoading ? <span className="p-1 text-[0.7rem] text-white bg-orange-600 rounded-2xl flex justify-center items-center gap-1">
                 <Loader/>
              </span> :
                <span onClick={()=>setOpenDeleteModal(true)} className="cursor-pointer p-1 text-[0.7rem] text-white bg-orange-600 rounded-2xl flex justify-center items-center gap-1">
     ...
     {
        openDeleteModal && <section className="w-full fixed top-0 left-0 h-screen bg-black/30 z-[99999] flex justify-center items-center font-outfit"
        onClick={()=>setOpenDeleteModal(false)}>
   
          <article className="p-5 w-full max-w-[450px] rounded-2xl bg-white" onClick={(e)=>e.stopPropagation()}>
            <h1 className="text-[0.9rem] font-normal">Are you sure want to delete this blog ?</h1>
